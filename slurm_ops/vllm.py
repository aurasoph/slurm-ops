...
# %% auto 0
__all__ = ['DEFAULT_SLURM_ARGS', 'DEFAULT_MODEL', 'SHARED_SIF_PATH', 'sif_exists', 'resolve_sif', 'build_sif', 'wait_for_discovery', 'vllm_up', 'vllm_down']
...
# %% ../nbs/02_vllm.ipynb 5
DEFAULT_SLURM_ARGS = (
    "--account=stf --partition=gpu-l40s --gres=gpu:1 "
...
def vllm_up(
    ...
    # 2. The salloc body — append our serve.sh after the slurm flags. Pass
    #    the resolved SIF path through to serve.sh via VLLM_SIF; srun
    #    propagates the env to the compute node by default.
    serve_cmd = f"VLLM_SIF={sif_path} srun --pty ~/{remote_vllm_dir}/serve.sh"
